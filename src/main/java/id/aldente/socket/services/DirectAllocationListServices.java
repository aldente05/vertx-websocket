 ...
 */
public class DirectAllocationListServices {


    private static final Logger logger = LoggerFactory.getLogger(DirectAllocationListServices.class);

    @NonBlocking
    public static BaseResponse<JsonArray> getDirectAllocationList(JSONObject jsonRequest, String dbeaqUrl, String dbeaqUser, String dbeaqPassword) {
        BaseResponse<JsonArray> result = new BaseResponse<JsonArray>();
        try {
            String kode = jsonRequest.getString("kode");
            String SQLGetPdTaskListClaimSharia = "SELECT * FROM eaq_dev.transaction_direct_allocation WHERE kode = ? ";
